...
/*   Updated: 2022/06/05 14:55:50 by lpaulo-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub.h"

static int	check_possibility_space(t_mode *mode, char c, int t_len, int a_len);
...
{
	if (check_possibility_space(mode, mode->map_file[p][x + 1], 0, 0) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p][x - 1], 0, 0) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p - 1][x], ft_strlen(mode->map_file[p - 1]), x)  == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p - 1][x + 1], ft_strlen(mode->map_file[p - 1]), x) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p - 1][x - 1], ft_strlen(mode->map_file[p - 1]), x) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p + 1][x], 0, 0) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p + 1][x + 1], 0, 0) == false)
		close_all(mode, INVALID_MAP);
	if (check_possibility_space(mode, mode->map_file[p + 1][x - 1], 0, 0) == false)
		close_all(mode, INVALID_MAP);
}
