...
/*   Updated: 2022/08/13 12:26:57 by lpaulo-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	horz_intersection(t_mode *mode, t_aux_ray *aux,
		t_utils_ray *utils);
/**
 * @brief intercept ponto de contato com fim do quadrado e step e o delta
 */
...
static void	vert_intersection(t_mode *mode, t_aux_ray *aux, t_utils_ray *utils)
{
	...
	{
		//forca um pixel para entrar na colisao e retornar true se tiver parede
		...
		//se nao tem colisao aumenta o delta/step
		else
		{
...
t_aux_ray	horizontal_ray(t_mode *mode, t_utils_ray *utils, float ray_angle)
{
	...
	//dependendo de onde o player esta olhando precisa multiplicar por -1 para
	//ter corretamente aonde o raio esta se nao pelo valor errado ele aponta para
	//direcao errada
	utils->y_step = TILE;
...
static void	horz_intersection(t_mode *mode, t_aux_ray *aux, t_utils_ray *utils)
{
	...
	{
		aux->xto_check = aux->next_touchx;
		if (utils->is_ray_facing_up == true)// forca a subtracao para que entre dentro da tile para quando ver se tem mapa la retornar true
		...
		else //se nao acharmos uma wall vms para o proximo incrementando pelo delta
		{
			aux->next_touchx += utils->x_step;
			aux->next_touchy += utils->y_step;
		}
